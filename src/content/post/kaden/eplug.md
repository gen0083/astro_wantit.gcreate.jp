...
<!-- textlint-disable -->
ePlugは家庭内の無線LANに接続して設定を行う。eHomeというスマホのアプリで設定する必要があるのは、以前購入した［eRemote-miniというスマートリモコン］(/eremote-mini/)と同じである。私の場合、すでにeHomeはインストールしているので、ePlugをデバイス追加するだけだった。
<!-- textlint-enable -->
...
<!-- textlint-disable -->
なぜこの機能を必要としたかは［二酸化炭素計の記事］(/co2_monitor/)を参照していただきたい。要約すると、この二酸化炭素計を設置する場所は寝室兼作業部屋なので、活動中は計測していたいが寝るときは止めておきたい。寝てる間に二酸化炭素濃度は上昇するので、最悪寝てるときにアラームが「ピピピ」と鳴り響いてしまう。しかしこの製品にはそれを防ぐような親切な機能がついていない。だから外部から強制的にタイマー機能を供給したというわけである。
<!-- textlint-enable -->
